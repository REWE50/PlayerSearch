...
{         
	...
	// the jQuery getJSON() method is called to parse a local JSON file, 'json/players.json'
	// this is a local file, but this method could also be used to return data from a remote web service/feed
	// this would require studying the web service's API to know exactly how to configure/call it
	...
	{ 
		...
		{            
			...
			// this list will be displayed in our left side panel only
			simpleListHTML += '<li>' + result.name + '</li>';
		...
		$("#clickPlayers").append(richListHTML).listview().listview('refresh');
...
// opens the left side panel on clicking the info icon
$("#panelIcon").click(function()
{
	$("#myLeftPanel").panel("open");
});
